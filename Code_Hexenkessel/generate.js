var L07_Hexenkessel;
(function (L07_Hexenkessel) {
    function generateContent(_data) {
        /* console.log(JSON.stringify(_data)); */
        var group = null;
        for (var category in _data) {
            var items = _data[category];
            group = createSelect(items, category);
            var div = document.querySelector("div#" + category);
            if (div && group)
                div.appendChild(group);
        }
        document.getElementById("Wirkung_value") ? .addEventListener("input", L07_Hexenkessel.slider) : ;
        document.getElementById("Zutaten_value") ? .addEventListener("input", L07_Hexenkessel.slider) : ;
    }
    L07_Hexenkessel.generateContent = generateContent;
    function createSelect(_items, _category) {
        var group = document.createElement("div");
        var select = document.createElement("select");
        select.id = _category;
        if (_category != "Zutaten") {
            var label = document.createElement("label");
            label.textContent = _category;
            label.htmlFor = _category;
            group.appendChild(label);
            group.innerHTML += "<br>";
        }
        for (var _i = 0, _items_1 = _items; _i < _items_1.length; _i++) {
            var item = _items_1[_i];
            var option = document.createElement("option");
            option.value = item.name;
            if (_category == "Zutaten") {
                option.setAttribute("price", (item.price).toString());
                option.setAttribute("stepper", (item.stepper) + "");
            }
            option.innerHTML = item.name;
            select.appendChild(option);
        }
        group.appendChild(select);
        var range = document.createElement("input");
        range.type = "range";
        range.min = "1";
        range.step = "1";
        range.id = _category + "_value";
        if (_category == "Wirkung") {
            range.name = "Wirkungsdauer";
            range.max = "60";
            var labelForRange = document.createElement("label");
            labelForRange.textContent = range.name;
            labelForRange.htmlFor = range.name;
            group.innerHTML += "<br>";
            group.appendChild(labelForRange);
        }
        else {
            range.name = "Menge";
            range.max = "500";
        }
        range.setAttribute("value", "30");
        group.innerHTML += "<br>";
        group.appendChild(range);
        var input = document.createElement("input");
        input.type = "text";
        input.id = "bubble" + range.id;
        input.setAttribute("value", "30");
        input.setAttribute("disabled", "disabled");
        input.setAttribute("class", "time");
        group.appendChild(input);
        if (_category == "Wirkung")
            group.innerHTML += " min";
        else
            group.innerHTML += " Stück/ml";
        return group;
    }
})(L07_Hexenkessel || (L07_Hexenkessel = {}));
